...
int main() {
    ...
    udp::endpoint mcast_endpoint(udp::v4(), 9000);
    udp::socket mcast_socket(ioc, mcast_endpoint);
    mcast_socket.set_option(
    ...
    udp::socket rtx_socket(ioc, udp::v4());
    udp::endpoint rtx_endpoint(net::ip::make_address_v4("192.168.29.226"), 8080);
    ...
    while (true) {
        size_t len = mcast_socket.receive_from(net::buffer(data), sender_endpoint);
        ...
        if(proto.sequence() != expected_seq) {
            std::array<char, 4> req;
            std::memcpy(req.data(), &expected_seq, sizeof(expected_seq));

            rtx_socket.send_to(net::buffer(req), rtx_endpoint);
        }
        expected_seq = proto.sequence() + 1;
