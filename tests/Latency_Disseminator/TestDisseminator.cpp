...
TestDisseminator::TestDisseminator(TestPublisher& publisher, Retransmitter& retransmitter) : 
DataDisseminator(publisher, retransmitter), publisher(publisher), retransmitter(retransmitter) {}

void TestDisseminator::start() {
    retransmitter.start();
    publisher.start();
}

void TestDisseminator::shutdown() {
    publisher.stop();
    retransmitter.stop();
}

void TestDisseminator::ingestEvent(const MarketDataEvent& event) {
    TimedBuffer tb;
    tb.ingest_ts = std::chrono::high_resolution_clock::now();

    auto buffer = serialize(event);
    tb.data = std::move(buffer);

    retransmitter.put(event.sequence, tb.data);
    publisher.push(tb);
}

inline std::vector<uint8_t> TestDisseminator::serialize(const MarketDataEvent& event) {
